"""
Integration tests for deterministic hash-based sampling feature in vLLM v0.9.2.

These tests verify:
1. The use_deterministic_hash parameter is accepted by the API
2. Outputs are 100% reproducible with the same seed
3. Different seeds produce different outputs
4. The feature works with various temperature and sampling parameters
...
def model_setup(inference_client: InferenceClient, urls: tuple[str, str]) -> str:
    """Setup a small model for testing deterministic sampling."""
    _, vllm_url = urls
...
def generate_with_deterministic_sampling(
    ...
    """Helper function to generate text with optional deterministic sampling."""
    ...
    if response.status_code != 200:
        logger.error(f"Error in API request: {response.text}")
...
def test_deterministic_sampling_reproducibility(urls: tuple[str, str], model_setup: str, test_prompt: str):
    """
    Test that deterministic sampling produces identical outputs across multiple runs.
    
    This is the core test for the deterministic hash-based sampling feature.
    """
    _, vllm_url = urls
    seed = 42
    temperature = 1.0  # Use non-zero temperature to test stochastic sampling
    
    # Generate 3 outputs with the same seed and deterministic sampling
    outputs = []
    logprobs_sequences = []
    ...
    for i in range(3):
        ...
        logger.info(f"Run {i+1}: {content[:50]}...")
    
    # All outputs should be identical
    assert outputs[0] == outputs[1], "Output 1 and 2 should be identical"
    assert outputs[1] == outputs[2], "Output 2 and 3 should be identical"
    
    # Logprobs should also be identical
    assert len(logprobs_sequences[0]) == len(logprobs_sequences[1])
    assert len(logprobs_sequences[1]) == len(logprobs_sequences[2])
...
def test_different_seeds_produce_different_outputs(urls: tuple[str, str], model_setup: str, test_prompt: str):
    ...
    assert output_seed_42 != output_seed_123, "Different seeds should produce different outputs"
...
def test_deterministic_vs_regular_sampling(urls: tuple[str, str], model_setup: str, test_prompt: str):
    """
    Compare deterministic and regular sampling.
    
    Note: Regular sampling with the same seed MAY produce the same output due to 
    PyTorch's RNG, but this is not guaranteed across platforms. This test just 
    verifies both modes work.
    """
    _, vllm_url = urls
    seed = 42
    temperature = 1.0
    ...
    # Generate with regular sampling
    ...
    logger.info(f"Deterministic output: {deterministic_output[:50]}...")
    logger.info(f"Regular output: {regular_output[:50]}...")
...
def test_deterministic_with_various_temperatures(urls: tuple[str, str], model_setup: str, test_prompt: str):
    ...
    for temp in temperatures:
        # Generate twice with same seed and temperature
        ...
        assert output1 == output2, f"Outputs should be identical at temperature {temp}"
...
def test_backward_compatibility_without_parameter(urls: tuple[str, str], model_setup: str, test_prompt: str):
    """
    Test backward compatibility: existing code without use_deterministic_hash should still work.
    """
    _, vllm_url = urls
    ...
    payload = {
        ...
        "logprobs": True,
        "top_logprobs": 3
        # NOTE: use_deterministic_hash is NOT included
    }
...
def test_deterministic_sampling_with_logprobs(urls: tuple[str, str], model_setup: str, test_prompt: str):
    ...
    for i, (pos1, pos2) in enumerate(zip(logprobs1, logprobs2)):
        assert pos1["token"] == pos2["token"], f"Token mismatch at position {i}"
        assert abs(pos1["logprob"] - pos2["logprob"]) < 1e-6, f"Logprob mismatch at position {i}"
...
def test_deterministic_sampling_with_longer_output(urls: tuple[str, str], model_setup: str):
    ...
    assert output1 == output2, "Longer outputs should also be deterministic"
