...
func (s *Server) handleExecutorRequest(ctx echo.Context, request *ChatRequest, w http.ResponseWriter) error {
	...
	// Stage-1 Sequence Check: Extract user_seed from request and generate run_seed
	// Per proposal: user_seed is provided by developer (e.g., seed: 42 in API request)
	userSeed := ExtractUserSeedFromRequest(&request.OpenAiRequest)
